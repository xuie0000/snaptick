...
internal fun calculateDayOfMonths(month: Int, year: Int): List<DayOfMonth> {
	...
	val month31day = (1..31).map { DayOfMonth(it.toString(), it, it - 1) }
	val month30day = (1..30).map { DayOfMonth(it.toString(), it, it - 1) }
	val month29day = (1..29).map { DayOfMonth(it.toString(), it, it - 1) }
	val month28day = (1..28).map { DayOfMonth(it.toString(), it, it - 1) }
