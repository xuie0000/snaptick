...
internal fun DefaultWheelTimePicker(
	...
	var hourScrollIndex by remember { mutableStateOf<Int?>(null) }
	var minuteScrollIndex by remember { mutableStateOf<Int?>(null) }
	var ampmScrollIndex by remember { mutableStateOf<Int?>(null) }
	var scrollGeneration by remember { mutableStateOf(0) }
	var lastExternalTime by remember { mutableStateOf(startTime.truncatedTo(ChronoUnit.MINUTES)) }

	LaunchedEffect(startTime) {
		val truncated = startTime.truncatedTo(ChronoUnit.MINUTES)
		if (truncated != lastExternalTime) {
			lastExternalTime = truncated
			snappedTime = truncated
			snappedAmPm = amPms.find { it.value == amPmValueFromTime(truncated) } ?: snappedAmPm
			hourScrollIndex = if (timeFormat == TimeFormat.HOUR_24) {
				hours.find { it.value == truncated.hour }?.index
			} else {
				amPmHours.find { it.value == localTimeToAmPmHour(truncated) }?.index
			}
			minuteScrollIndex = minutes.find { it.value == truncated.minute }?.index
			ampmScrollIndex = amPms.find { it.value == amPmValueFromTime(truncated) }?.index
			scrollGeneration++
		}
	}
...
internal fun localTimeToAmPmHour(localTime: LocalTime): Int {
	...
	if (isBetween(localTime, LocalTime.of(0, 0), LocalTime.of(0, 59))) {
		return localTime.hour + 12
	}

	if (isBetween(localTime, LocalTime.of(1, 0), LocalTime.of(11, 59))) {
		return localTime.hour
	}

	if (isBetween(localTime, LocalTime.of(12, 0), LocalTime.of(12, 59))) {
		return localTime.hour
	}

	if (isBetween(localTime, LocalTime.of(13, 0), LocalTime.of(23, 59))) {
		return localTime.hour - 12
	}
